#!/bin/bash
# Create Debian VM Script
# Creates a Debian VM on Proxmox with PrivateBox services pre-installed
#
# Exit codes:
#   0 - Success
#   1 - General error
...
if [[ ! $DEBIAN_VERSION =~ ^[0-9]+$ ]]; then
    log_error "Invalid Debian version format: $DEBIAN_VERSION"
...
function download_image() {
    ...
    echo "Debian cloud image downloaded successfully to cache."
...
{
    echo "========================================="
    echo "Starting Debian ${DEBIAN_VERSION} VM Creation"
    echo "========================================="
    echo "VM ID: ${VMID}"
