...
run_integration_tests() {
    ...
    if ssh "${USERNAME}@${SERVER}" "ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no operator@${vm_ip} 'echo VM is accessible' 2>/dev/null"; then
    ...
    if ssh "${USERNAME}@${SERVER}" "ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no operator@${vm_ip} 'test -f /etc/privatebox-cloud-init-complete' 2>/dev/null"; then
        ...
        log_info "  ssh ${USERNAME}@${SERVER} \"ssh operator@${vm_ip} 'sudo cloud-init status'\""
    fi
