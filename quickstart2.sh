#!/bin/bash
# PrivateBox Quick Start Script v2
# 
# Simple installation method for PrivateBox with improved bootstrap
# 
# Recommended usage (safer):
#   curl -fsSL https://raw.githubusercontent.com/Rasped/privatebox/main/quickstart2.sh -o quickstart2.sh
#   sudo bash quickstart2.sh [options]
#
# One-line usage:
#   curl -fsSL https://raw.githubusercontent.com/Rasped/privatebox/main/quickstart2.sh | sudo bash
#
# Available options:
...
print_banner() {
    echo -e "${BLUE}==========================================="
    echo "     PrivateBox Quick Start v2"
    echo "==========================================="
    echo -e "${NC}"
}
...
run_bootstrap() {
    ...
    if [[ ! -f "$bootstrap_script" ]]; then
        ...
        if [[ "$DRY_RUN" == true ]]; then
            error_exit "Original bootstrap doesn't support --dry-run"
        fi
    fi
